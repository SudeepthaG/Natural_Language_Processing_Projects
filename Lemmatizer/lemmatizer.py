    ...
    if re.search('\t', line):
        ...
        if form in lemma_count:
            if lemma in lemma_count[form]:
                count=lemma_count[form][lemma] +1
                # lemma_count.pop(form)
                lemma_count[form][lemma]= count
            else:
                lemma_count[form][lemma]=1
        else:
            lemma_count.update({form: {lemma: 1}, })
print(lemma_count)

print("wordform types:")
